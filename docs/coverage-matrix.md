...
6. **Phase 5 concurrency emit is partial.** Channels plus synchronous send/receive/close, `go`, and `scope` now emit (`samples/Channels.gs`, `samples/GoScope.gs`). `select`, `async` / `await`, and `await for` remain interpreter-only pending the async-aware lowering/state-machine decision (ADR-0027); the aspirational samples still cover those surfaces on the interpreter.
