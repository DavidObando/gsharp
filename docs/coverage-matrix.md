...
| `type Name enum { A, B, C }` | ✅ | ✅ | — | ✅ | Phase 6.8. Members auto-number from 0. Underlying type is `System.Int32`. `Color.Red` member access; equality only (no ordering). Usable as switch discriminant in both the statement and expression forms. Explicit member values (`Red = 5`) and emit are deferred. |
