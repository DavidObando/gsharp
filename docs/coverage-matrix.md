...
| `class` declaration | ✅ | ✅ | ✅ | ✅ | Phase 3.B.3 (sub-step 1): `type Name class { Field Type … }` declares a reference-typed aggregate. Field declarations and composite literal `Name{Field: …}` parse and bind identically to `struct`, but assignment shares the instance (reference semantics) rather than deep-copying, and field writes mutate in place so all references observe the update. Emit lowers the type as a CLR sealed reference type with a synthesized parameterless `.ctor`, the literal as `newobj` followed by `dup; <value>; stfld` per initializer, and field access / assignment through the heap reference (no `ldloca`/copy dance). Methods inside the body, parameterized constructors, single inheritance, and `open` / `override` (ADR-0017) land in subsequent sub-steps. |
