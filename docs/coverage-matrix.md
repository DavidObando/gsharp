...
| `async func` / `await e` | ✅ | ✅ | ✅ | ✅ | Phase 5.1+5.2 / ADR-0023 + ADR-0034. `async` is a `func` modifier; the call-site return type is wrapped as `Task` / `Task[T]`. `await` is an expression that unwraps the awaited element type. The interpreter realizes an async function as `Task.FromResult(body-result)` and `await` blocks via `GetAwaiter().GetResult()`. Emit mirrors that synchronous semantics exactly: async CLR signatures return `Task` / `Task<T>`, returns wrap with `Task.CompletedTask` / `Task.FromResult<T>`, and `await` emits `GetAwaiter().GetResult()` with a scratch awaiter local. |
...
6. **Phase 5 concurrency emit covers the core surface.** Channels plus synchronous send/receive/close, `go`, `scope`, `select`, `async`, and `await` now emit (`samples/Channels.gs`, `samples/GoScope.gs`, `samples/Select.gs`, `samples/AsyncTask.gs`). Only `await for` remains interpreter-only pending the async-aware lowering follow-up in ADR-0034.
...
| Pure async/await sample interoperating with BCL `Task` | ✅ both | `samples/AsyncTask.gs` — `async func` + `await` against `Task.Delay`, driven from a top-level `scope { go ... }`, now covered by the emit conformance harness per ADR-0034. |
| Coverage matrix ✅ on the entire concurrency section | ✅ interp / ✅ emit (except `await for`) | Channels + go + scope + select + async/await all emit; only `await for` remains emit-deferred pending ADR-0034's follow-up. |
