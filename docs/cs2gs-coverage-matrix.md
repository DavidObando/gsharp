...
| CatchFilterClause | CatchFilterClauseSyntax | ADR-0115 §B.27 |  | tools/cs2gs/corpus/grid/G03-ControlFlow-Console/Constructs/CatchFilterClause.cs |  | Translated to a native G# `when` filter (ADR-0177), which the emitter lowers to a real CLR filter region — first-pass timing and fall-through to a later sibling are the runtime's, so the #1724/#2235 overlapping-sibling gap is closed. A filter needing hoisted statements (a scrutinee spill or pattern designation) is not a single G# expression and still lowers inside the handler; that residue is reported unsupported when a later sibling overlaps. |
...
| ExtensionBlockDeclaration | ExtensionBlockDeclarationSyntax | ADR-0115 §B.19 |  | tools/cs2gs/corpus/grid/G13-Extensions-Console/Constructs/ExtensionBlockDeclaration.cs | https://github.com/DavidObando/gsharp/issues/1879 | C# 14 `extension(T x)`/`extension(T)` block members map onto the same target as a classic this-param extension method (ADR-0115 §B.19): an instance method/property lowers to a receiver-clause func (a property becomes a get-only func, since G#'s prop grammar has no receiver clause, with call sites rewritten to a zero-arg call); enum/owned receivers use ADR-0165's explicit extension receiver form; a static member becomes a plain shared member of the declaring class, with call sites rewritten from the extended type's name to the real owner. A settable instance extension property remains an explicit gap (grid G13). |
...
| IndexerDeclaration | IndexerDeclarationSyntax | ADR-0115 §B.11 |  | tools/cs2gs/corpus/grid/G07-Members-Console/Constructs/IndexerDeclaration.cs |  | User indexers (issue #944). A `ref`/`ref readonly`-returning indexer gaps loudly at the declaration site: G#'s by-ref return is a `func` feature (issue #490 / ADR-0060) and there is no `prop this[...] ref T` spelling, so translating it would silently return a copy (issue #3839). |
...
| PropertyDeclaration | PropertyDeclarationSyntax | ADR-0115 §B.11 |  | tools/cs2gs/corpus/grid/G07-Members-Console/Constructs/PropertyDeclaration.cs |  | A `ref`/`ref readonly`-returning property gaps loudly at the declaration site: G#'s by-ref return is a `func` feature (issue #490 / ADR-0060) and there is no `prop P ref T` spelling, so translating it would silently return a copy (issue #3839). |
