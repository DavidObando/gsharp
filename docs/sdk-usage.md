...
The SDK satisfies the managed `CompileDesignTime` contract used by
`dotnet-watch`, reruns the normal reference and `gsgen` pipelines after each
edit, emits a candidate assembly, and applies System.Reflection.Metadata EnC
deltas to the running module. Method-body edits include changed control flow,
string literals, locals, existing member calls, referenced G# projects, and
generated sources whose metadata shape stays stable.
...
G# currently emits metadata and IL deltas, but not Portable PDB deltas.
Updated code executes without restarting; existing breakpoint and stepping
locations remain bound to the baseline PDB. Restart after edits that move
sequence points or change local-variable layout when debugger accuracy matters.
...
| [`samples/HotReload`](../samples/HotReload) | Local, transitive project-reference, and Avalonia-style generated `gsgen` hot-reload scenarios. |
