...
3. **(Shipped in Stream B′ / ADR-0036) Event subscription via `+=` / `-=`.** Originally deferred because `+=` desugared at parse time only for bare-identifier LHS. Now: a dedicated `EventSubscriptionExpressionSyntax` is recognised in `ParseAssignmentExpression` whenever the LHS is an `AccessorExpressionSyntax`, the binder resolves the trailing identifier as an `EventInfo` (static via `ImportedClassSymbol`, instance via `clrType.GetEvent`), and add/remove dispatch reaches `EventInfo.AddEventHandler` (interpreter) and `add_X` / `remove_X` accessor IL (emit). Function-literal handlers are materialized against the event's declared delegate type. Multi-segment LHS (`a.b.c.Event += h`) falls out naturally.
