...
Add three ergonomic surfaces for `data struct`: method-style `.copy(F = v, …)`, positional deconstruction `let (a, b) = p` plus named deconstruction `let { F = a, G = b } = p`, and `let p2 = p with { F = v }`. ADR-0168 later adds mutable positional `var (a, b) = p` and mixed inline bindings in multi-assignment. `.copy(...)` is parser-recognized sugar scoped to data-struct receivers; named arguments are accepted in the general argument parser but the binder diagnoses them unless the callee is exactly `.copy` on a data struct. `with` is a contextual keyword recognized only as `expr with { … }`, so `with` remains usable as an identifier elsewhere.
