...
|---|---|---|
| Return type | Identical (normalized source text) AND identical resolved type | Same textual convention ADR-0144 §E uses for base clauses / type-parameter lists. `int32` vs `Int32` is a mismatch — stricter than name resolution, relaxable later. The text check alone is not enough: each part's return type is bound against its OWN file's imports (Copilot review round 7), because an identical spelling can still resolve to two different types across files — an unimported/unresolved short name, or two distinct same-named types from different namespaces. |
