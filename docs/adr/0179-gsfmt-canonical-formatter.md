...
| LS formatting tests cannot be validated — migrated `LanguageServer.Tests` sat close to the flat 10-minute parity budget (#3931) | low | Largely retired: the mirrored-test budget is now derived from the app's declared `[Fact]` count rather than fixed for the whole corpus (#3501), so `LanguageServer.Tests` is budgeted for the tests it actually has (~16 minutes today) and each test added raises its budget with it. Adding tests to that project no longer turns its parity numbers into truncation artifacts |
