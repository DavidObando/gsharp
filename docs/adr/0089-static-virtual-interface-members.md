...
**Further revision (issue #2945): nullable erasure for static-virtual properties.** Static-virtual property matching now uses the same CLR slot rule as instance interface properties. Reference-nullable wrappers, including `T?` over an unconstrained or class-constrained type parameter, are erased from the declared interface slot before generic substitution; therefore a get-only `T?` slot may be implemented by `T`. A concrete nullable value type (`int32?`) and `T?` where `T : struct` retain `System.Nullable<T>` and cannot be implemented by `T`. Settable properties remain invariant. Static accessor `MethodImpl` rows and constrained property reads use that same declared-slot projection, so accepted programs load and dispatch through the CLR slot.
