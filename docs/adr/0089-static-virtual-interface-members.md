...
There are two distinct concepts here. **Static-virtual identity** — a per-implementer operation or constant dispatched through a generic constraint — is expressed with a `func` or a `prop` (issue #1019): `prop X T { get; }` implemented by each type's `shared { prop X T { get { … } } }`. **Shared static state** — a single storage location owned by the interface itself, not the implementer — is a `var` / `let` / `const` field (issue #1030). The latter is *not* per-implementer: every reader sees the same field on the interface TypeDef. CLR interfaces may own static fields, so non-generic interface static state maps directly to `Static` FieldDef rows (with a synthesized `.cctor` for initializers and `const` literals inlined). Generic-interface static state remains rejected with GS0330 — a generic interface's static field would need per-construction storage with `TypeSpec`-parented field references at every access site, which is out of scope for this release. The `event` member kind inside an interface `shared { … }` block is also still rejected with GS0330 (see "Diagnostics").
...
| ID | Severity | Trigger |
| --- | --- | --- |
| **GS0330** | Error | A non-`func`, non-`prop`, non-field member (`event`) appears inside an interface `shared { … }` block, **or** a `var`/`let`/`const` field appears inside a *generic* interface's `shared` block (issue #1030: generic interface static storage is out of scope). Non-generic interface static `var`/`let`/`const` state is now accepted (issue #1030). The diagnostic names the owning interface and points to this ADR. |
...
| --- | --- | --- |
| Interface static state on a **generic** interface (`var`/`let`/`const` in `interface IBox[T] { shared { … } }`) | Deferred (issue #1030) | A generic interface's static field would need per-construction storage and `TypeSpec`-parented field references at every access site. The non-generic case (the issue's motivating shape) is fully supported; the generic case is rejected with a refined GS0330. |
...
Non-generic interface static `var`/`let`/`const` state and default-bodied static-virtual `prop` members are both accepted (issue #1030). A `var`/`let`/`const` field inside a *generic* interface's `shared` block, and any non-`func`/non-`prop`/non-field member (e.g. `event`), are rejected with GS0330.
