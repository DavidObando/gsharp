...
- **Phase 3 (usability):** Delimiter-aware scanner; `${e,align:format}`; multiline holes; Tier 2 generalized; new diagnostics GS0222–GS0225 (the originally-proposed GS0212–GS0216 codes were already taken); parser span remapping. Update `docs/lexical.md`, `docs/diagnostics.md`.
...
- **Phase 3 (grammar) — done.** A delimiter-aware splitter (`Parser.SplitHole`) extracts the expression / alignment / format clauses, tracking `()[]{}` depth and top-level conditional-expression nesting while skipping nested string/char literals and comments, so `${n.GetType()}`, `${dict["k"]}`, and `${cond ? "a" : "b"}` are not mis-split. A later colon may still introduce formatting (`${cond ? 1 : 2:D4}`). Invalid (non-constant) alignment reports **GS0220**. The **lexer hole scanner** (`Lexer.ScanInterpolationHole` / `SkipInterpolationNestedLiteral`) is now delimiter-aware end-to-end: it tracks `()[]{}` nesting, skips nested `"…"`/`'…'` literals (recursively, so an inner interpolation works) and `//` / `/* */` comments, and **permits newlines**, so indexer holes (`${dict["k"]}`), ternary holes (`${cond ? "a" : "b"}`), and multiline holes lex correctly. The malformed-hole diagnostics are implemented as **GS0222** (unterminated hole), **GS0223** (empty hole), **GS0224** (empty format), and **GS0225** (newline in literal portion) — the ADR's originally-proposed GS0212–GS0216 codes were already taken.
- **Phase 3 (tooling) — done.** The parser remaps each hole's tokens, nodes, and diagnostics to **absolute outer-file spans** (`Parser.BuildHolePaddedText` over the original `SourceText`), and `InterpolatedStringExpressionSyntax.HoleExpressions` makes the hole sub-trees reachable from `GetChildren`. On top of that: `SemanticTokensComputer` decomposes an interpolated literal into `String` filler plus the hole expression tokenized as real code; and hover, go-to-definition, find-references, rename, completion, and signature-help all work inside `${ … }`. The TextMate grammar (`gsharp.tmLanguage.json`) was rewritten for sigil-free G# (interpolation inside an ordinary `"…"`, `$ident`/`${…}`, `$$`/`""` escapes, non-interpolating backtick raw strings). See `docs/lsp.md` and `docs/vscode-gsharp-spec.md`.
- **Phase 3+ (Tier 3 handler lowering) — done (#368).** On the emit path only, `InterpolatedStringHandlerLowerer` rewrites each node into a `BoundBlockExpression` that constructs a `System.Runtime.CompilerServices.DefaultInterpolatedStringHandler` value-type local, calls `AppendLiteral`/`AppendFormatted<T>` per part, and yields `ToStringAndClear()`. The interpreter is untouched, so there is no interpret/compile drift.
  ...
  - **Value-type holes do not box (#368 criterion 5).** `AppendFormatted<T>` is closed over the hole's concrete CLR type via a MethodSpec, so e.g. an `int32`/`float64` hole is passed by value. **Historical note (superseded by ADR-0087 R1–R7):** at the time this ADR was written, a *user-defined* G# type that had no reflection `System.Type` was passed through the generic method closed over an `object` placeholder, with the real type-argument symbol encoded into the emitted MethodSpec (issue #320 mechanism); the value was still passed by its natural representation (no box), and the user TypeDef — not `object` — was what the verifier saw. Under the reified emit, the user TypeDef is a real `TypeDef`+`GenericParam` shape and the MethodSpec carries the user type directly, so the `object` placeholder is no longer needed.

  - **`[InterpolatedStringHandlerArgument]` forwarding (#368 criterion 4) — done.** An interpolated string passed to a parameter typed as a *user* `[InterpolatedStringHandler]` now binds to a handler-carrying `BoundInterpolatedStringExpression` (see `InterpolatedStringHandlerInfo`) and lowers to a `BoundBlockExpression` that constructs the user handler with `(literalLength, formattedCount, …forwarded args[, out bool shouldAppend])`, appends each part (gated by the `out bool`/`bool`-returning short-circuit shape when present), and yields the constructed handler value itself. Forwarded names map to preceding sibling arguments by parameter name (`""` forwards the receiver). Overload resolution ranks the string→handler conversion last (`ImplicitConversionKind.InterpolatedStringHandler`) so a real `string` overload always wins. Failures report **GS0221**. The emitter honours `out`/`ref` constructor arg ref-kinds (byref signature encoding in `GetCtorReference`). The interpreter has parity via `Evaluator.EvaluateInterpolatedStringHandler`. **Byref handler parameters (`ref T` / `in T`) are also supported (#377 sub-item 1):** `InterpolatedStringHandlerInfo.TryCreate` peels the parameter ref-kind, the binder bypasses the byref-source check for handler arguments, and the lowerer wraps the trailing handler local in a `BoundAddressOfExpression` when the parameter is byref (typical for `ref struct` handlers like `DefaultInterpolatedStringHandler`). **Forwarded arguments are evaluated exactly once (#377 sub-item 2):** the binder now captures each non-side-effect-free forwarded source into a local *before* constructing the handler, reuses the captured value both in the original parameter slot and in the handler constructor, and prepends the capture-assignments as a prelude on a `BoundBlockExpression`, matching the C# spec single-evaluation requirement.
  - **`await` inside a hole (`"x=${await f()}"`) — done.** When any hole contains an `await`, `InterpolatedStringHandlerLowerer` pre-evaluates every hole value into a leading temporary *before* the (ByRefLike) handler is constructed, so no handler local is live across a suspension. The async `SpillSequenceSpiller` was taught to flatten a `BoundBlockExpression` (the lowered interpolation) so the leading `tmp = await …` statements are spilled like any other top-level await. The interpreter already evaluated awaits in holes directly; the emit path now matches. **The gated user-handler shape (`out bool`/`bool` short-circuit) is also covered (#377 sub-item 3):** because hole values are pre-spilled into temporaries before any gate-check append call is emitted, an `await` inside a hole composes with the conditional-goto gate without leaving an awaitable live across the gate's labels.

### Resolved follow-ups (#377)

The five corner cases originally tracked under #377 are now closed:

1. **Byref handler parameter shapes** — `ref T` / `in T` / `out T` handler parameters are accepted and the lowerer emits the constructed handler local by address. Common case: `StringBuilder.Append(ref DefaultInterpolatedStringHandler)`.
2. **Single-evaluation of `[InterpolatedStringHandlerArgument]`-forwarded arguments** — captured at the call site into a temp shared between the original parameter slot and the handler constructor.
3. **`await`-in-hole + gated user handler** — composes correctly because the hole pre-spill happens before the gate's conditional-goto block.
4. **`M(object)` vs `M(FormattableString)` tiebreak** — `OverloadResolution.CompareConversions` now prefers `InterpolatedStringToFormattable` over a reference conversion to `System.Object` for an interpolated-string argument, matching C# behavior.
5. **Named-argument-wrapped interpolations** — `ComputeInterpolatedStringArgFlags` (both the binder copy and the `ImportedClassSymbol` copy) now unwraps `NamedArgumentExpressionSyntax` so `M(arg: $"…")` target-types to `FormattableString` just like the positional form.

### Deferred / not yet covered

(None remaining for the Phase 3+ scope tracked by #368/#369/#377.)
