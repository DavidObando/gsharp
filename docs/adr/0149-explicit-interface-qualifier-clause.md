...
`TranslateMethod`/`TranslateProperty`/`TranslateIndexer`/`TranslateExplicitEvent` all compute the qualifier's `GTypeReference` with the same `typeMapper.Map(...)` helper already used for base-interface-list translation and emit the plain member name plus the clause — never a mangled name. Properties, indexers, and events use this path for both source and imported CLR interfaces; the binder resolves imported clause targets against `ImplementedClrInterfaces`, and the emitter writes `MethodImpl` rows for their reflected accessor slots. This lets same-named imported-interface members with different types coexist instead of dropping the later declaration (#3535). Methods retain the narrower pre-existing external-interface fallback. A field-like C# event declaration (`event Handler Name;`) can never carry an explicit interface specifier in C# itself, so `TranslateEventField` is unaffected — only the custom add/remove accessor form needs the clause-based path.
