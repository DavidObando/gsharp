...
- Two narrower, pre-existing gaps are intentionally **out of scope** for this ADR (neither is a regression — neither ever worked before, for any indexer/event, explicit or not):
  - **Static explicit-interface members** (`shared func (IFoo) M()`, etc.) are not supported — resolving them would require new binder passes over `StaticMethods`/`StaticProperties`/`StaticEvents` distinct from the instance-member passes this ADR extends.
  - **Interface-typed receiver call-site access** for indexers (`asIface[i]`) and events (`asIface.Changed += h`) does not bind (`GS0158`/"not indexable") — `ExpressionBinder.Access.cs`'s indexer/event lookup only resolves a `StructSymbol`/concrete-type receiver. Ordinary interface PROPERTY and METHOD access through an interface-typed receiver already works; only indexers and events share this narrower call-site limitation. This affects any interface indexer/event access (explicit or ordinary), not just explicit implementations, and does not block declaring or implementing (even explicitly) an interface indexer/event — only accessing one indirectly through an interface-typed variable.
