...
### J. Non-MSBuild callers: gsc's own `/analyzer:` flag (issue #2215)

§F's `_GsharpRunSourceGenerators` only fires from MSBuild, so any caller that
invokes gsc directly — the cs2gs migration pipeline's compile stage, tests, a
hand-rolled build script — got no generator output at all, even when it already
knew the project's resolved analyzer assembly paths. Rather than teach every
such caller to shell out to `gsgen` itself and stitch the result into its own
gsc invocation, gsc grew a repeatable `/analyzer:<path>` flag: when one or more
are supplied, gsc — still Roslyn-free itself, per ADR-0027 — spawns `gsgen` as
...
When `/analyzer:` is absent (the overwhelmingly common case — every existing
gsc invocation), gsc does not spawn a process at all: a single flag check before
its normal compile path, the same fast-path-guard discipline as §F's
`@(GsharpAnalyzer)`-empty skip and #2214's stray-`.cs` skip.

This also fixed a latent gap in `gsgen` itself, exposed only once a caller other
than the SDK's fully-resolved-reference-closure `GsgenTask` invoked it: the C#
stub compilation `GsgenProgram.BuildMetadataReferences` builds for generator
execution (§B) had no fallback to the host's trusted-platform-assembly set when
`/r:` was sparse or absent, unlike `ReferenceResolver.WithReferences` (used for
G# binding), which already falls back. A minimal `/analyzer:`-only gsc
invocation therefore produced a stub with too few — sometimes zero — references,
silently breaking every `ForAttributeWithMetadataName` match (even
`System.ObsoleteAttribute` failed to resolve). Fixed by exposing
`ReferenceResolver.HostTrustedPlatformAssemblyPaths()` and using it as the same
fallback in `BuildMetadataReferences`.

**cs2gs wiring.** `Cs2Gs.ProjectLoading.CSharpProjectLoader` now also resolves a
loaded project's own `<Analyzer>` reference paths
(`Project.AnalyzerReferences`), and `TranslateStage` forwards the **primary**
app's own paths (not a referenced sibling project's — a sibling's generators
already ran in its own migration run) into `StageExecutionContext`, which
`CompileStage` passes to `GscInvoker.Compile` as `/analyzer:`/`/gsgentool:`.
Two subtleties fell out of wiring this up against a real generator, both fixed
in the shared `Cs2Gs.Translator` core (so they benefit every caller, not just
this one):

- `CSharpProjectLoader.BuildDocuments` already excludes a generator-produced C#
  document from translation (it recognizes the standard `<auto-generated>`
  header / `obj`-directory location — the same heuristic that already skips
  Designer.cs-style codegen). But the **symbol-level** partial-parts merge
  `CSharpToGSharpTranslator` performs for issue #1910 (merging every C#
  `partial` fragment of a type into one G# declaration) walks
  `INamedTypeSymbol.DeclaringSyntaxReferences` across the **whole compilation**,
  which still includes that excluded document's syntax — so the excluded
  generator's members were silently merged in anyway, duplicating whatever
  gsc's own `/analyzer:`-triggered gsgen run adds afterward (`GS0102`). Fixed by
  a new optional `retainedFilePaths` filter on `CSharpToGSharpTranslator`: when
  supplied (cs2gs passes the project's own kept `Documents`), only declaring
  parts from files still in that set are merged; a type left with fewer than
  two eligible parts is no longer treated as "partial" for merge purposes.
- The merged G# type's `partial` modifier itself is dropped by the default
  cs2gs merge policy (issue #1910: cs2gs owns every fragment, so the merged
  result is a complete, non-partial type). That is wrong when a *real*
  additional fragment is still coming from gsc's own gsgen run: gsc requires
  every fragment of a merged type to carry `partial` (exactly as C#/G# do), so
  the translated type must stay `partial` too. Fixed by a new
...
- **Non-MSBuild callers** (§J, issue #2215) — gsc's `/analyzer:`/`/gsgentool:`
  flags and sibling-process `gsgen` invocation, the `BuildMetadataReferences`
  host-TPA-fallback fix, and the cs2gs `TranslateStage`/`CompileStage`/
  `GscInvoker` wiring (with the two `CSharpToGSharpTranslator` fixes —
