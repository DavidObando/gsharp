...
#### B.17 Explicit casts → checked conversion calls — ADR-0049, ADR-0167

A C# explicit cast `(T)expr` maps to the G# **checked conversion call** `T(expr)`; a nullable target maps to `T?(expr)`. Numeric examples include `(int)x` → `int32(x)` and `(byte)n` → `uint8(n)`; reference downcasts use CLR `castclass` semantics, preserving null and throwing `InvalidCastException` for an incompatible non-null value. C# `expr as T` remains G# `expr as T`, the nullable testing conversion from ADR-0160. Numeric narrowing is unchanged: the CLR truncates a `float64`→`int32` conversion toward zero exactly as C# `(int)` does, so `(int)Math.Floor(d*100.0 + 0.5)` remains bit-for-bit faithful.
