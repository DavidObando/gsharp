...
A C# `try`/`catch`/`finally` maps to the canonical G# `try { } catch (e T) { } finally { }`, **clause for clause** — since [ADR-0177](0177-catch-clause-parity.md) G# catch clauses are at C# parity, so no clause merging or synthesis is needed. A **typed** catch `catch (FooException ex)` becomes `catch (ex FooException)` (binder name first, then type — the receiver-clause ordering); a **type-only** catch `catch (FooException)` becomes `catch (FooException)`, binding nothing; a bare `catch { }` stays a bare `catch { }`; and a `when` filter becomes a native G# `when` filter. Pattern designations introduced by the filter are visible in its handler, so `catch (E ex) when (ex.InnerException is T value)` translates directly with no spill or in-handler fallback. The emitter lowers filters to real CLR filter regions, preserving first-pass timing and fall-through; `finally` carries over verbatim. (Before ADR-0177 a bare `catch` did not parse and `catch (T)` silently bound a *variable named `T`* that caught everything, so the translator had to synthesize `__caught`-style binders and merge overlapping filtered siblings into one hand-replayed dispatch tree — issue #3897 family 1, issues #1724 and #2235.) `throw new FooException(args)` maps to `throw FooException(args)` (object construction, §B.16). A C# **bare re-throw** `throw;` maps to G# `rethrow` ([ADR-0176](0176-rethrow-statement.md), issue #3897), which emits `ILOpCode.Rethrow` and preserves the exception's original throw site. Before ADR-0176 G# had no rethrow spelling, so this lowered to `throw ex` over the caught binder — the same exception instance, but with its `StackTrace` reset to the re-throw site.
