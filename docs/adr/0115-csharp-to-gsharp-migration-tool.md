...
The mapping is recorded as an Info diagnostic. An entry class that declares a nested aggregate type is preserved instead, with `Main` and its sibling static methods in the class's `shared { }` block. An executable project that another project in the same migration run compiles against (a declared compile `ProjectReference`, i.e. not `ReferenceOutputAssembly="false"`) is likewise preserved (issue #3645): its entry class is consumable API surface — a referencing project calls it as an ordinary CLR type (`GsgenProgram.Run(...)`) — and flattening would erase the type from the migrated assembly, producing `GS0157` at every cross-project use site. G# supports a class-scoped static `Main`; retaining that form preserves the nested type's declaring owner and accessibility, while ordinary nested translation preserves its generic context. Only the type that *contains the entry point* is otherwise hoisted; other `static` utility classes still map to a class whose members sit in a `shared { }` block (§B.11, ADR-0053). This applies to executable compilations (a library compilation has no entry point, so its static classes keep the `shared { }` mapping).
...
#### B.17 Explicit casts → checked conversions — ADR-0049, ADR-0167

A C# numeric/value cast `(T)expr` maps to the G# conversion call `T(expr)`; a nullable value target maps to `T?(expr)`. A C# reference cast maps to the constructor-independent `cast[T](expr)` form (`cast[T?](expr)` for a nullable-annotated target), preventing an applicable `T(object)` or `T(Base)` constructor from constructing a new object instead of casting. Reference downcasts use CLR `castclass` semantics, preserving null and throwing `InvalidCastException` for an incompatible non-null value. C# `expr as T` remains G# `expr as T`, the nullable testing conversion from ADR-0160. Numeric narrowing is unchanged: the CLR truncates a `float64`→`int32` conversion toward zero exactly as C# `(int)` does, so `(int)Math.Floor(d*100.0 + 0.5)` remains bit-for-bit faithful.
...
A C# `try`/`catch`/`finally` maps to the canonical G# `try { } catch (e T) { } finally { }`, **clause for clause** — since [ADR-0177](0177-catch-clause-parity.md) G# catch clauses are at C# parity, so no clause merging or synthesis is needed. A **typed** catch `catch (FooException ex)` becomes `catch (ex FooException)` (binder name first, then type — the receiver-clause ordering); a **type-only** catch `catch (FooException)` becomes `catch (FooException)`, binding nothing; a bare `catch { }` stays a bare `catch { }`; a `when` filter becomes a G# `when` filter, which the emitter lowers to a real CLR filter region so first-pass timing and fall-through match C#; `finally` carries over verbatim. (Before ADR-0177 a bare `catch` did not parse and `catch (T)` silently bound a *variable named `T`* that caught everything, so the translator had to synthesize `__caught`-style binders and merge overlapping filtered siblings into one hand-replayed dispatch tree — issue #3897 family 1, issues #1724 and #2235. All of that is gone; the only residue is a filter that needs hoisted statements, such as a pattern designation, which cannot be a single G# expression and still lowers inside the handler.) `throw new FooException(args)` maps to `throw FooException(args)` (object construction, §B.16). A C# **bare re-throw** `throw;` maps to G# `rethrow` ([ADR-0176](0176-rethrow-statement.md), issue #3897), which emits `ILOpCode.Rethrow` and preserves the exception's original throw site. (Before ADR-0176 G# had no rethrow spelling, so this lowered to `throw ex` over the caught binder — the same exception instance, but with its `StackTrace` reset to the re-throw site. The filtered-catch `when` lowering and the merged multi-clause dispatch tree's no-clause-matched fallthrough use `rethrow` for the same reason.)
