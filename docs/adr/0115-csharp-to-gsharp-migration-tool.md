...
- **Get-only auto-property + explicit constructor (Oahu.Decrypt OD-T1 / issue #2746).** When a single-instance-constructor `class` assigns a parameter to any property, the translator does **not** lift to a primary constructor: G# primary-constructor parameters are fields, so lifting renames the constructor parameter after the property and removes the CLR property getter. Instead it keeps the explicit `init(...)` and emits each affected C# get-only auto-property (`{ get; }`) as a G# **init-only** property `prop X T { get; init; }` (a bare `{ get; }` is read-only and the kept `init` assignment would be `GS0127`). An inline get-only-auto-property initializer (`{ get; } = new();`) — which G# cannot express as a property member initializer — is moved into the explicit constructor body. Plain `struct`s (which admit no explicit `init`) and field-only class constructors still use the primary-constructor lift.
