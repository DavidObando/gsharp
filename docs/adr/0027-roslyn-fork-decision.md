...
GSharp v1.0 ships on the bespoke `ReflectionMetadataEmitter`. Issue #51 is **closed as `wontfix` for v1.0**, with a note that revival remains possible if any of its four triggers materialises after v1.0. The vendored `src/Roslyn` submodule and the dependent `src/CodeAnalysis/` stub project have now been removed from the repository (see "Removal" below); if any of the issue #51 triggers later applies, the submodule can be re-added pointing back at the `DavidObando/gsharp-roslyn` fork at the corresponding tag.
...
If any of these forgone capabilities later become a release blocker, the path back to #51 is preserved: the vendored `src/Roslyn` submodule and the `src/CodeAnalysis/` stub project can both be reintroduced from history (their last point in `main` is referenced from the follow-up removal PR), and the bespoke emitter can remain behind a feature flag during migration (as #51 already prescribes). Closing #51 today is not the same as deleting the option permanently; it is choosing not to spend Phase 7 on infrastructure that is not on the critical path for v1.0.
...
**Delete `src/Roslyn` outright as part of this ADR.** Adopted as a follow-up. The tree was large and its removal touched `.gitmodules`, the CI workflow, and the `src/CodeAnalysis/` stub project; it was best handled in a dedicated PR so the diff stayed reviewable. As of that follow-up the submodule, the stub project, and the related signing key under `build/keys/` are gone from `main`; the CI workflow no longer fetches submodules or builds a Roslyn fork. The fork repo (`DavidObando/gsharp-roslyn`) is preserved unchanged in case any of the issue #51 triggers later materialises.
