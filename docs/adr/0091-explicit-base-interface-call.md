...
- `base.M()` (no `[`) parses as before, i.e. as `NameExpression("base").M()`, which falls into the ordinary identifier-lookup channel and surfaces the existing "name 'base' not in scope" diagnostic.
...
| `base[BaseClass].M(...)` (class base, not interface) | Deferred | G# already supports `base(args)` for constructor chaining (ADR-0010) but does not yet have a general "call my base class's M" syntax. That is a different feature (issue tracker only). |
