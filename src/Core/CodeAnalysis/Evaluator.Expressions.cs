    ...
    private object EvaluateExpression(BoundExpression node)
    {
        try
        {
            return node.Kind switch
            {
                ...
                BoundNodeKind.StackAllocExpression => throw EvaluatorException.CreateCompiledOnlyStorageBoundary("stackalloc is not supported in the interpreter; it requires the CIL localloc emit path.", node),
                ...
                BoundNodeKind.SizeOfExpression => throw EvaluatorException.CreateCompiledOnlyStorageBoundary("sizeof on an unmanaged-pointer struct pointee is not supported in the interpreter; it requires the CIL sizeof emit path.", node),
                BoundNodeKind.FunctionPointerFromMethodExpression => throw EvaluatorException.CreateCompiledOnlyStorageBoundary("'&Method' function pointers are not supported in the interpreter; they require the CIL ldftn/calli emit path (ADR-0122 §9).", node),
                BoundNodeKind.FunctionPointerInvocationExpression => throw EvaluatorException.CreateCompiledOnlyStorageBoundary("function-pointer invocation ('fp(args)') is not supported in the interpreter; it requires the CIL calli emit path (ADR-0122 §9).", node),
