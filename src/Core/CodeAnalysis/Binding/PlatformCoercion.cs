...
internal static class PlatformCoercion
{
        ...
        string boundary)
    {
        ...
        // simply flows on, which is byte-for-byte the behaviour that existed
        // before §4 was implemented. The ADR describes this mode as strictly
        // weaker than either model and explicitly not a supported one.
        if (!NullabilityOptions.PlatformNilChecksEnabled)
        {
            return expression;
        }
