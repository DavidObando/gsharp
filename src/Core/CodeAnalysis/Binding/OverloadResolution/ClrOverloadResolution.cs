...
#pragma warning restore SA1201
    ...
    {
        {
                        ...
                        if (gmi.GetParameters().Any(static p =>
                                ...
                                && ClrTypeUtilities.IsDelegateType(
                                    Invariant.Required(
                                        ...
                                        "an array parameter has an element type"))))
                        {
            ...
            for (var i = 0; i < argTypes.Count; i++)
            {
                ...
                var conv = symbolicUserDefinedImplicitConversionCheck?.Invoke(i, paramTypes[i]) == true
                    ? ImplicitConversionKind.UserDefinedImplicit
                    : ClassifyImplicit(paramTypes[i], argTypes[i], supplementaryInterfaceCheck);
                ...
                if (conv == ImplicitConversionKind.None)
                {
                    ...
                    else if (openLiteralArgumentCheck?.Invoke(i, paramTypes[i]) == true
                        && IsErasedGenericParameterSlot(rawCandidate, paramIndex, explicitTypeArgIsGenuine))
                    {
                        conv = ImplicitConversionKind.StructuralProjection;
                    }
        ...
        where T : MethodBase
    {
        ...
        for (var i = 0; i < argTypes.Count; i++)
        {
            ...
            paramTypes[i] = target;
            var conv = symbolicUserDefinedImplicitConversionCheck?.Invoke(i, target) == true
                ? ImplicitConversionKind.UserDefinedImplicit
            ...
            if (conv == ImplicitConversionKind.None)
            {
                ...
                else if (openLiteralArgumentCheck?.Invoke(i, target) == true
                    && IsErasedGenericParameterSlot(rawCandidate, slot, explicitTypeArgIsGenuine))
                {
                    conv = ImplicitConversionKind.StructuralProjection;
                }
    ...
    /// be bound.
    /// </summary>
    private static bool TryInferTypeArgumentsForExpandedParams(
        MethodInfo openMethod,
        IReadOnlyList<Type?> argTypes,
        IReadOnlyList<string?>? argumentNames,
