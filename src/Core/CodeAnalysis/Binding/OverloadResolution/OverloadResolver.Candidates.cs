...
internal sealed partial class OverloadResolver
{
        ...
        ImmutableArray<TypeSymbol> explicitTypeArguments = default)
    {
        ...
        foreach (var cand in applicable)
        {
            ...
            // same reason the carrier does, one line further down. Ranking
            // `Elem[T](values ...T)` against `Elem(values ...object)` for
            // `Elem("a", "b")` compared `string` to the OPEN `T` (which erases
            // to object) and to `object`, landed both candidates on the same
            // conversion kind, and let the later non-generic-over-generic
