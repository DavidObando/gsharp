...
/// <para>Four methods, not eight. Until issue #3877 the directional handles
/// each needed a <c>Channel&lt;T&gt;</c> twin, because an imported generic
/// extension on a <c>ChannelReader&lt;T&gt;</c> was member-not-found on the
/// <c>chan[T]</c> every caller actually writes (ADR-0174 errata 32/41). The
/// receiver now views along the D2 lattice like any other operand, so the
/// twins are gone and <c>ch.ReceiveBatch(…)</c> reaches the reader form
/// directly.</para>
