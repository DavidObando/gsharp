...
Every command-line driver uses the emitter, including the interactive `gsi` REPL. `/out:<path>` controls whether `gsc` saves the assembly or runs the emitted program immediately. The legacy tree-walking evaluator, its `Compilation.Evaluate` API, and the `--engine evaluator` / `GSI_ENGINE=evaluator` escape hatches have been removed.
