...
Numeric operators are defined for matching primitive types; G# does not silently promote arbitrary mixed numeric operands. Use explicit conversions when crossing type families. Constant integer literals are target-typed like C#: an in-range literal can initialize or assign to a narrower integer type without a cast (`var b uint8 = 42`), while out-of-range constants and non-constant narrowing still require an explicit conversion and may be rejected.
...
let triple = [3]int32{1, 2, 3}
let zeros = [10]int32          // runtime/zero-initialized []int32
let names = map[int32,string]{1: "one", 2: "two"}
```

The `[n]T` expression allocates a fresh zero-initialized slice of runtime length `n`; `[n]T{...}` keeps the fixed-count initializer check, and `[]T{...}` infers the length from the initializer. Nullability position matters for arrays and slices: `[]T?` means a non-nil slice whose elements are nullable, `[]?T` means the slice itself is nullable, and `[]?T?` means both. The same rule applies to `[N]T`.

Unsafe code can use raw pointers (`*T`) inside an `unsafe` context. Generic low-level code can constrain `T` with `unmanaged` and use `sizeof(T)`:

```gsharp
unsafe func bytesFor[T unmanaged](count int32) int32 {
...
Any CLR collection type can be constructed and populated in a single expression with a brace-enclosed element list after the construction target. This is G#'s equivalent of C# collection initializers (`new List<int>{1, 2, 3}`) and the named-type counterpart to Swift's `[…]` literals.
