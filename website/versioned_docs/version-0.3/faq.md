...
G# uses width-bearing fixed-size integer names such as `int8`, `uint16`, `int32`, and `uint64` so the type's size is visible in source and consistent with names like `float32` and `float64`. These replaced the earlier C#-style integer names while the language was still pre-stable; `nint` and `nuint` remain the native-width integer spellings. Friendly aliases (`int`, `uint`, `long`, `ulong`, `short`, `ushort`, `byte`, `sbyte`, `float`, `double`) resolve to the canonical names in the binder. Diagnostics, `typeof`, hover, and IL still print the canonical width-bearing spelling, which remains preferred in documentation and public APIs.
...
Yes for supported native `.gsproj` builds. The SDK runs `gsgen`, an out-of-process Roslyn source-generator host, before `gsc` and includes the generated `.g.gs` files in the compile. Direct `gsc` users can pass already-resolved generator assemblies with `/analyzer:<path>` and, if needed, `/gsgentool:<path>`.

## Is there a C# to G# migration tool?

Yes. `cs2gs` translates C# projects into canonical G# and runs a Translate → Compile → IL-verify → Test-parity pipeline for gap discovery. See [C# to G# migration (`cs2gs`)](./tooling/cs2gs).
