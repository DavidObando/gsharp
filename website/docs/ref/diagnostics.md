...
### Pointer / by-ref diagnostics (GS9001–GS9011)
...
| GS0588 | Error | A `ref`-returning function cannot be converted to a delegate or function-type value; call it directly instead. A ref-returning function LITERAL is only supported as `let name = func (...) ref T { ... }`, a directly-callable local function (issue #4219). | `let at = func(xs []int32, i int32) ref int32 { return ref xs[i] }` followed by `let cb = at` (bare-name delegate conversion), or passing a `func (...) ref T { ... }` literal as an argument, or `var at = func (...) ref T { ... }` (only a `let` local-function declaration is the direct-call shape). |
| GS0589 | Error | A struct member cannot return a reference to its own instance state; `this` is implicitly `scoped`. Mark the member `@UnscopedRef` (import System.Diagnostics.CodeAnalysis) to allow it (ADR-0184, the CS8170 analogue). | `struct Acc { var Total int32; func slot() ref int32 { return ref this.Total } }` — add `import System.Diagnostics.CodeAnalysis` and `@UnscopedRef` above `func slot`. |
| GS0590 | Error | `@UnscopedRef` is only meaningful on a non-`shared`, non-`override` instance member declared in a struct body; ADR-0184 D4 also defers `override` and interface members. | `@UnscopedRef` on a receiver-clause func, a free function, a class member, a `shared` member, an `override`, or an interface member. |
| GS0591 | Error | Cannot return a reference obtained through a read-only receiver (an `in` parameter, a `ref readonly` alias, or a `ref readonly` result): the receiver is defensively copied into function-local storage before the call, so the returned reference would point into that copy (ADR-0184 amendment, the CS8156 analogue). | `func fromIn(in a Acc) ref readonly int32 { return ref a.Slot() }` where `Slot` is `@UnscopedRef` — take the receiver by `ref` instead of `in`, or return the value. |
| GS0592 | Error | A tuple-destructuring arrow-lambda parameter pattern (ADR-0185) requires at least two elements — G# has no 1-tuples. | `let f = ((x int32)) -> x` — add a second element, or use an ordinary parameter (`(x int32) -> x`) instead. |
| GS0607 | Error | `partial` is not valid here; only a `func` member of a `partial class` or `partial struct` may be partial (ADR-0192). | `partial prop P int32 { … }`, `partial func F()` at top level, `partial func` on an interface member, or a partial method with a receiver clause or explicit-interface qualifier (`partial func (IFoo) Bar()`) — all out of scope. |
| GS0608 | Error | A partial method must be declared inside a `partial class` or `partial struct` (ADR-0192; the C# CS0751 analogue). | `class A { partial func F() int32; … }` — mark the enclosing type `partial`. |
...
| GS9011 | Error | A `ref`/`var ref` LOCAL alias of the enclosing function cannot be captured by a closure because the closure may outlive the storage it aliases. | `func Foo() { var arr = []int32{1}; var ref alias = arr[0]; var f = func() { alias = alias + 1 } }` — `f` captures the ref-alias local `alias`. |
...
Storage-dependent unsafe constructs were compiled-only in the former tree
evaluator (`GS0513` for unmanaged `&`/`*`, plus legacy `GS9999` guards).
The evaluator and its diagnostics have been deleted: every driver executes
...
| GS0242 | _Retired_ | Previously: an argument written without `in` at an `in` parameter was rejected. Since issue #4400 it is passed by readonly reference, as in C# — the address of an lvalue of the parameter type, otherwise the address of a readonly temp holding the (implicitly converted) value; this diagnostic is no longer emitted. |
...
- **GS0242** (retired) — `f(x)` where `f(in x int32)` used to be rejected. It now binds like C#: `f(x)` passes `x` itself, and `f(x + 1)` or `f(someInt32)` at an `in int64` parameter passes a readonly temp holding the converted value. An argument with no implicit conversion to the parameter type reports the ordinary GS0154, and `ref`/`out` parameters still require their modifier (GS0235).
...
- All three codes apply verbatim to the **`if let` expression** (`let x = if let v = maybe { v } else { fallback }`): the terminal `else` is required, each branch block must end in a value, and the tails must unify. A non-nullable binding initializer additionally reports GS0296.
...
`(T1, T2,...) -> R` the canonical function-type clause spelling — the type spelling now matches the lambda expression form. The legacy `func(...) R` and `async func(...) R` type-clause spellings remain accepted for one release to ease migration, but every legacy occurrence produces a warning. The managed function-pointer form `*func(...) R` is distinct and is not deprecated.
...
Named arguments are written `name: value`. The legacy `name = value` spelling — deprecated with a
...
| GS0583 | Error | `Attribute '<name>' has no constructor that accepts the argument list (<argument types>). Change the arguments to match one of its constructors (issue #4097; C# spells the same rule CS1729 and CS1503).` | `@Many([]object{1})` where `ManyAttribute` declares only `ManyAttribute(Type[] values)` — before #4097 the whole attribute was dropped from the assembly with no diagnostic |
| GS0584 | Error | `Constructor parameter '<name>' of user-defined attribute '<attribute>' has type '<type>', which is not a valid attribute parameter type. An attribute constructor parameter must be a primitive, string, System.Type, object, an enum, or a 1-D array thereof (issue #4097; C# spells the same rule CS0181, and reports it on the attribute's declaration rather than on each use).` | `class BadAttribute(Value Widget) : Attribute` applied as `@Bad(nil)`, where `Widget` is a class — `csc` reports CS0181 on the same shape. Unreachable for this shape as of #4143: GS0585 now rejects the declaration before any use exists |
| GS0585 | Error | `Attribute constructor parameter '<name>' has type '<type>', which is not a valid attribute parameter type. An attribute constructor parameter must be a primitive, string, System.Type, object, an enum, or a 1-D array thereof (issue #4143; C# spells the same rule CS0181).` | `class BoxedAttribute(Value Holder) : Attribute` where `Holder` is an ordinary class — reported on the class's own declaration, even if the attribute is never applied anywhere |
...
interpreter. The evaluator — and with it this diagnostic — was deleted;
every driver executes emitted code, where P/Invoke runs natively.

## Interpreter deinitializer boundary (GS0510, retired)

GS0510 warned that a `deinit` finalizer would not run under interpreted
execution. The evaluator — and with it this warning — was deleted;
deinitializers run as real CLR finalizers on every driver.
...
the parenthesized nullable form `(chan T)?`: the slot zero
...
Declaring an uninitialized `chan T` local is allowed,
...
zero values (ints, strings, structs, and the magic collections)
...
`= default` initializer can reintroduce nil into a bare slot. Recursive
collection defaults close the other former corner — a bare struct-typed local's own
...
was deleted; every driver executes emitted code, where ByRefLike values work
natively.
