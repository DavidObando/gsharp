...
| GS0584 | Error | `Constructor parameter '<name>' of user-defined attribute '<attribute>' has type '<type>', which the attribute encoder cannot write: a same-compilation type has no CLR type while the attribute blob is built. Use a primitive, string, System.Type, an imported enum, or a 1-D array thereof.` | `class NoteAttribute(Kind Status) : Attribute` applied as `@Note(Status.Active)`, where `Status` is declared in the same compilation |
