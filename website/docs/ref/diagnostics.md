...
| GS0593 | Error | `@GeneratedRegex` (ADR-0187) requires a non-empty constant string as its `pattern` argument. | `@GeneratedRegex(somePattern) func Foo() Regex;` where `somePattern` is not a constant, or a bare `@GeneratedRegex` with no arguments. |
| GS0594 | Error | `@GeneratedRegex` (ADR-0187) is not valid on this declaration's shape (a body, parameters, generics, `async`, an extension, a `ref` return, or a return type other than `Regex`). | `@GeneratedRegex("^a$") func Foo(x int32) Regex;` — a `@GeneratedRegex` function takes no parameters. |
| GS0595 | Error | `@GeneratedRegex` (ADR-0187) combines `IgnoreCase` (explicit or via an inline `(?i)` group) with culture-sensitive matching; this cannot be lowered to a cached `Regex` without changing matching semantics. | `@GeneratedRegex("(?i)^a$") func Foo() Regex;` — add `RegexOptions.CultureInvariant` (and drop any `cultureName`), or remove `IgnoreCase`. |
| GS0596 | Error | `@GeneratedRegex` (ADR-0187) failed a compile-time construction probe: an invalid pattern, an unsupported `RegexOptions` combination, or a non-`-1` negative `matchTimeoutMilliseconds`. | `@GeneratedRegex("^a$", RegexOptions.ECMAScript | RegexOptions.Singleline) func Foo() Regex;` — `ECMAScript` cannot combine with `Singleline`. |
