...
| GS0600 | Error | `partial` is not valid here; only a `func` member of a `partial class` or `partial struct` may be partial (ADR-0192). | `partial prop P int32 { … }`, `partial func F()` at top level, or `partial func` on an interface member — partial properties, top-level partial functions, and partial interface members are all out of scope. |
| GS0601 | Error | A partial method must be declared inside a `partial class` or `partial struct` (ADR-0192; the C# CS0751 analogue). | `class A { partial func F() int32; … }` — mark the enclosing type `partial`. |
| GS0602 | Error | A partial method has no implementing part; every partial method declared in G# must be implemented by exactly one part with a body (ADR-0192 §B). G# deliberately diverges from C#, which silently elides an unimplemented `void` partial method and its call sites. | `partial class A { partial func F() int32; }` with no second part — add the implementing part, or drop `partial` and give the declaration a body. |
| GS0603 | Error | A partial method must have exactly one signature-only declaring part and one implementing part with a body (ADR-0192). | Two parts that both have bodies, two parts that are both signature-only, or an implementing part with no declaring part (the C# CS0759 analogue). |
| GS0604 | Error | The declaring and implementing parts of a partial method disagree on some aspect of the signature — return type, parameter names/types/modifiers, accessibility, `open`/`override`, `async`/`suspend`, `ref` return, or type parameters (ADR-0192 §D). | `partial func F(x int32) int32;` paired with `partial func F(y int32) int32 { … }` — the parameter names differ. |
