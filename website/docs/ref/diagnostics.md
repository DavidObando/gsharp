...
| GS0578 | Error | `Circular constraint dependency involving '<type-parameter>' and '<type-parameter>'. A type parameter cannot, directly or through a chain of type-parameter constraints, be constrained by itself.` | `class Cyc[A B, B A]` — `A` is constrained by `B`, which is constrained by `A` (C# spells the same rule CS0454) |
