...
`let` communicates immutability of the binding. `var` introduces a mutable variable. `const` is for compile-time constants. Tuple deconstruction and named deconstruction use `let` forms. Multi-target assignment is statement syntax and is limited to identifier target lists today. The legacy `identifier ":=" Expression` short variable-declaration form was removed; use `let name = expr` (immutable) or `var name = expr` (mutable) instead.
