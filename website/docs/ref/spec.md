...
func Make[T class init()]() T { return T() }                // class + init(): T() constructs T (issue #988, renamed from new() by #997)
...
Function literals use `func` or `async func`. A trailing lambda can appear after an explicit call close-paren and is desugared into the final argument. ADR-0074 (issue #714) also introduces a dedicated arrow-lambda expression form: `(p1, p2) -> body`. Parameter types and the return type are **inferred** from the target delegate type (ADR-0076, ADR-0119); they may also be spelled explicitly (`(p1 T1, p2 T2) -> body`). A single-parameter lambda may drop the parentheses (`x -> body`, issue #932). The body is either a single expression or a brace-delimited block. A block body is a statement block with an optional trailing value expression (ADR-0128 / issue #1172): an `if`-without-`else` and other void control-flow run as statements, `return` works anywhere, and the trailing expression (or `return`) supplies the lambda value (void when neither yields a value) — full parity with `func` literals. Arrow lambdas share the function-literal capture, lowering, and emit pipeline (closures, `ldftn`/`newobj`, delegate construction). The inferred-type arrow lambda is the **canonical** lambda form (ADR-0119); the explicit `func (x T) R { … }` literal remains the alternative for when explicit typing is desired or no target type is available. A parameter named `_` is a **discard** (issue #1262): more than one `_` parameter may appear in a single parameter list — on lambdas and on named functions/methods alike (e.g. an event handler `(_, _) -> …`) — and the repeated-name check that otherwise reports `GS0101` does not apply to `_`. Each `_` still occupies its own positional slot (so the emitted signature has the correct arity), but a discard parameter is **not** added to the body's lookup scope, so referencing `_` does not resolve to a parameter. A non-`_` duplicate parameter name (`(x, x) -> …`) is still rejected with `GS0101`.
...
ConstraintFlag    ::= 'class' | 'struct' | 'init' '(' ')'  (* repeatable flag constraints, ADR-0097; 'init()' renamed from 'new()' by issue #997 *)
Parameters        ::= Parameter (',' Parameter)*
