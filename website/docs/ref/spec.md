...
The prefix `*T` is **context-sensitive** (ADR-0122 / issue #1014). **Outside** an `unsafe` context it denotes a *managed* by-ref pointer (CLR `ELEMENT_TYPE_BYREF`, `T&`): GC-tracked, legal only as a ref-kind parameter (`ref`/`out`/`in`), a `ref` return, or a `let ref` local — and rejected as a field type (`GS9006`) or plain parameter type (`GS0243`). **Inside** an `unsafe` context the same `*T` denotes an *unmanaged* raw pointer (CLR `ELEMENT_TYPE_PTR`, C# `T*`): not GC-tracked, and legal as a field, local, and plain (non-ref-kind) parameter type. An `unsafe` context is entered by an `unsafe` modifier on a function (`unsafe func F(...)`), an `unsafe { … }` block statement, or an `unsafe` modifier on a type declaration (`unsafe class` / `unsafe struct`); `unsafe` is a contextual keyword (no new reserved word). The legal pointee subset is the blittable primitives (`int8`…`int64`, `uint8`…`uint64`, `nint`/`nuint`, `float32`/`float64`, `bool`, `char`) and pointers-to-pointers; the opaque `void*`/`byte*` form is spelled `*uint8`. A pointer to a managed reference type or non-blittable pointee is rejected with `GS0398`. Inside an unsafe context the unmanaged pointer supports dereference read/write (`*p`), indexing (`p[i]` ≡ `*(p + i)`), scaled arithmetic (`p + i` / `p - i`), comparison/equality, a null pointer (`nil`), casts between pointer types and to/from `nint`/`IntPtr` (the `(byte*)x` / `(void*)x` / `(nint)p` round-trip), and use as a plain `@DllImport`/`@LibraryImport` parameter. These operations are lowered in the binder to native-int arithmetic and reinterpret conversions, so no new node kinds are introduced. Genuinely-unsafe pointer IL is *unverifiable by design* (as in C#); the emit tests assert runtime behaviour. Pointer difference (`p - q`), a true `void*` element type distinct from `*uint8`, pointers to user structs, function pointers, and fixed-size buffers are deferred follow-ups; `fixed`/pinning is implemented separately (ADR-0125 / issue #1026, described below).

`stackalloc T[n]` stack-allocates a contiguous buffer of `n` elements of a blittable element type `T`, emitted as the CIL `localloc` instruction and reclaimed when the method returns (ADR-0124 / issue #1024). It has two forms, selected by the target type exactly as the unsafe gating of `*T` falls out of context. The **safe** form is the default and needs **no** `unsafe` context: `var buf = stackalloc uint8[4]` yields a `System.Span[T]` (or `System.ReadOnlySpan[T]` when the target is one) over the allocated block, so element access is bounds-checked and `.Length` is available. The **unsafe** form is reached only inside an `unsafe` context when the target type is a `*T` unmanaged pointer (`var p *int32 = stackalloc int32[3]`), yielding the raw `localloc` pointer; outside an unsafe context no pointer target exists, so the safe `Span[T]` form is always chosen and no extra "unsafe required" diagnostic is needed. The count is a full expression (a runtime length is allowed). The element type must be a blittable primitive or a pointer; a managed/non-blittable element type is rejected with `GS0399`. The buffer is **zero-initialised** (method bodies are emitted with `.locals init`, which zeroes `localloc` memory, matching C# safe-`stackalloc`). `stackalloc` is a contextual keyword recognised only in the `stackalloc T[…]` shape, so it remains usable as an ordinary identifier elsewhere. `localloc` IL is *unverifiable by design* (as in C#); the emit tests assert runtime behaviour. The initializer form (`stackalloc T[n]{…}`) is a deferred follow-up.
