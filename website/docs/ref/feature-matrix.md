...
| Exact CLR arrays | Supported | Supported | `[]T` / `array[T]` and `[N]T` preserve CLR-array identity, existing covariance restrictions and copying ranges. The Go-style built-ins remain retired. |
| Native shared-storage slices | Supported (.NET 10) | Supported (emitted backend) | `slice[T]` / `readonly slice[T]`: shared endpoint-based views, independent length/capacity, append, overlap-safe copy, addressable value elements, readonly access, native pattern rests and exact-array/memory interop (ADR-0190). Suspended element writes are GS0602; persistent handles and Go fixed-value arrays are separate. |
...
| Receiver methods and extension functions | Supported | Supported | G# receiver style and imported CLR extension dispatch. A receiver clause is unconditionally an extension — for BCL/imported types and for types this package owns (classes, structs, enums) alike; there is no ownership-based branch and no marker keyword. In-body declarations are the only spelling for a real owned-type instance method. |
...
| Named delegate types | Supported | Supported | `delegate X(...);` declares a real CLR `MulticastDelegate`-derived type; generic delegates (`delegate X[T any](...);`) are supported. The retired `type`-based form reports `GS0535`. |
...
| Capturing rich anonymous objects | Supported | Supported | Rich fields infer heap-storable types, initializers/base arguments evaluate at the literal site, and free-variable member uses share lexical cells. Generated rich constructors initialize snapshots/capture state before the selected base call without changing named-class order. |
...
| Explicit structural interface adaptation | Supported | Supported | `adapt[I](source)` emits a fresh exact forwarding wrapper; `adapt[I](ref handle)` retains an ADR-0188 value-type location. Methods, exact generic/ref contracts, properties/indexers/events, inherited/default slots, identity/null rules, imported types, and direct AOT-friendly calls are supported. Static abstract/virtual members and operators diagnose with `GS0606`. |
