...
| P/Invoke/`extern` | Supported | Supported (emit-only) | Attribute-driven via `@DllImport("lib")` on a `;`-body `func` (ADR-0086 / issue #727), or via the source-generator-shaped `@LibraryImport("lib", StringMarshalling: …)` (ADR-0092 / issue #758, AOT-friendly with an explicit IL stub). v1 marshals primitives, `string`, `*T` (byref), and slices of primitives. |
