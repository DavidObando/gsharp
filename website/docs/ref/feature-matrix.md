...
| Variadic parameters (`name ...T`) | Supported (top-level `func` only) | Supported (top-level `func` only) | ADR-0101 / issue #799. Canonical Go-style spelling `name ...T`; body sees `[]T`; at most one variadic per signature and must be last (`GS0145`, `GS0364`). Call site packs N trailing args into a fresh `[]T`; a single trailing `[]T` argument passes through unwrapped (identity preserved). The emitter stamps `[System.ParamArrayAttribute]` so C# / F# / VB consumers see it as `params T[]`. The C# `params` keyword is rejected with `GS0363` pointing at the canonical form. Class methods, interface methods, constructors, lambdas, and delegates continue to report `GS0146` until a follow-up ADR lifts the restriction. |
...
| `default(T)` and bare `default` literal | Supported | Supported | ADR-0100 / issue #795. `default(T)` for any type expression; bare `default` valid in target-typed positions (`let`/`var` with explicit type, `return` with known return type, typed call argument, `?:` branch typed by sibling). Diagnostic `GS0362` when no target type is available. |
...
| Attributes | Supported | Semantically recognized | Includes `@Attribute` sugar and `@Obsolete`; `@DllImport` opts into P/Invoke (ADR-0086); `@LibraryImport` opts into the source-generator-shaped P/Invoke (ADR-0092). |
| P/Invoke/`extern` | Supported | Supported (emit-only) | Attribute-driven via `@DllImport("lib")` on a `;`-body `func` (ADR-0086 / issue #727), or via the source-generator-shaped `@LibraryImport("lib", StringMarshalling: …)` (ADR-0092 / issue #758, AOT-friendly with an explicit IL stub). v1 marshals primitives, `string`, `*T` (byref), slices of primitives, and blittable / explicit-layout structs via `@StructLayout(LayoutKind.…)` + `@FieldOffset(N)` (ADR-0093 / issue #759). `ref` / `out` / `in` parameters are supported for blittable pointees (primitives and `@StructLayout` structs) per ADR-0094 / issue #760 — the runtime marshals the byref slot as `T*` to the unmanaged callee. Function-pointer marshalling (ADR-0095 / issue #761) supports both managed delegate callbacks (`@UnmanagedFunctionPointer(CallingConvention.Cdecl)` on the delegate type) and raw `unmanaged[Cdecl] (T) -> R` function pointers (encoded as `ELEMENT_TYPE_FNPTR` in metadata). Per-parameter `@MarshalAs(UnmanagedType.…)` overrides (ADR-0096 / issue #762) opt a parameter into a different unmanaged form (`LPWStr` for Windows `…W` entry-points, `LPUTF8Str` for modern C APIs, `I4` to widen a `bool` to a C `int` flag, `LPArray` with `SizeParamIndex:` for sibling-sized buffers, etc.). |
