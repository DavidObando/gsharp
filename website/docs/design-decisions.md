...
| [0015](https://github.com/DavidObando/gsharp/blob/main/docs/adr/0015-multi-target-assignment.md) | Multi-target assignment evaluation order | Captures all target components first, evaluates RHS values once, then writes left-to-right; supports storage targets and a tuple-valued single RHS. |
...
| [0184](https://github.com/DavidObando/gsharp/blob/main/docs/adr/0184-unscoped-ref-definition-side.md) | Definition-side `@UnscopedRef` — ref-safe-context of `this` in struct members | Makes `@UnscopedRef` a real, type-identity-resolved attribute that lets a struct member (`func`, `prop`, or indexer) return a reference into its own instance state, and emits it to CLR metadata. Adds `GS0589` (the CS8170 analogue) and `GS0590` (placement). Amends ADR-0058 §4, whose `[UnscopedRef]` enforcement never actually governed ref returns, and corrects its rule that `return this` by value from a `ref struct` member needs the annotation. |
...
| ADR | Title |
| --- | --- |
| [0157](https://github.com/DavidObando/gsharp/blob/main/docs/adr/0157-default-tostring-synthesis.md) | Display-side structural formatting for REPL values |
| [0158](https://github.com/DavidObando/gsharp/blob/main/docs/adr/0158-synchronization-story.md) | Synchronization through `Gsharp.Extensions.Sync` and `SyncMap[K,V]` |
| [0159](https://github.com/DavidObando/gsharp/blob/main/docs/adr/0159-magic-collection-zero-values-and-nil-comparison.md) | Sound collection zero values and uniform nil comparison |
| [0160](https://github.com/DavidObando/gsharp/blob/main/docs/adr/0160-as-operator-yields-nullable.md) | The `as` operator yields a nullable type |
| [0161](https://github.com/DavidObando/gsharp/blob/main/docs/adr/0161-retire-equals-named-argument-separator.md) | Retire the `=` named-argument separator |
| [0162](https://github.com/DavidObando/gsharp/blob/main/docs/adr/0162-boolean-is-patterns.md) | Full patterns in boolean `is` expressions |
