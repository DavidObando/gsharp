...
public static class GNodeSamples
{
    internal static bool IsConcreteNodeType(Type type) =>
        ...
        && type != typeof(GNode)
        && type != typeof(GExpression)
        && type != typeof(GStatement)
        && type != typeof(GMember)
        && type != typeof(GPattern)
        && type != typeof(GTypeReference)
        && !type.IsAbstract;
    ...
    private static IReadOnlyDictionary<Type, Func<CompilationUnit>> Build() =>
        new Dictionary<Type, Func<CompilationUnit>>
        {
            ...
            [typeof(AnonymousClassMemberInitializer)] = () => Expr(new AnonymousClassLiteralExpression(
            ...
            [typeof(WhileLetStatement)] = () => Stmts(new WhileLetStatement(
                new[] { new IfLetBinding("v", Id("maybe")) },
                new BlockStatement(new GStatement[] { new ExpressionStatement(Id("v")) }))),
