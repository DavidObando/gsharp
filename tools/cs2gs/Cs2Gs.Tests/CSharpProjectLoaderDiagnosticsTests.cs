...
");
    ...
    /// <summary>
    /// Repository migration includes checked-in generated files while retaining
    /// the historical diagnostic-loader default that excludes them.
    /// </summary>
    [Fact]
    public async Task LoadProjectAsync_IncludeAutoGenerated_KeepsCheckedInDesigner()
    {
        string projectDir = NewScratchDir("checked-in-generated");
        File.WriteAllText(Path.Combine(projectDir, "Directory.Build.props"), "<Project></Project>");
        string designerPath = Path.Combine(projectDir, "Resources.Designer.cs");
        File.WriteAllText(
            designerPath,
            "// <auto-generated />\npublic static class Resources { public static string Name => \"x\"; }");
        string projectPath = Path.Combine(projectDir, "Generated.csproj");
        File.WriteAllText(
            projectPath,
            "<Project Sdk=\"Microsoft.NET.Sdk\"><PropertyGroup>" +
            "<TargetFramework>net10.0</TargetFramework></PropertyGroup></Project>");

        LoadedCSharpProject diagnostic = await CSharpProjectLoader.LoadProjectAsync(projectPath);
        LoadedCSharpProject repository = await CSharpProjectLoader.LoadProjectAsync(
            projectPath, includeAutoGenerated: true);

        Assert.DoesNotContain(
            diagnostic.Documents,
            document => string.Equals(document.FilePath, designerPath, StringComparison.OrdinalIgnoreCase));
        Assert.Contains(
