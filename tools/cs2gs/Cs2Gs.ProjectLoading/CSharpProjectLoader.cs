...
/// <list type="bullet">
/// <item><description>
/// <see cref="LoadProjectAsync"/> — the primary
...
#pragma warning restore RS2008
    ...
    /// <param name="includeAutoGenerated">
    /// Whether checked-in files carrying an auto-generated header are included.
    /// Files under build output directories remain excluded.
    /// </param>
    /// <param name="includedAutoGeneratedPaths">
    /// When supplied, limits auto-generated-header inclusion to these inventoried paths.
    /// </param>
    ...
    public static async Task<LoadedCSharpProject> LoadProjectAsync(
        ...
        IReadOnlyCollection<string> includedAutoGeneratedPaths = null)
    {
        ...
        return await BuildLoadedProjectAsync(
            project,
            loadDiagnostics,
            cancellationToken,
            includeAutoGenerated,
            includedAutoGeneratedPaths)
            .ConfigureAwait(false);
    }
        ...
        IReadOnlyCollection<string> includedAutoGeneratedPaths = null)
    {
        ...
        {
            results.Add(await BuildLoadedProjectAsync(
                referenced,
                new List<Diagnostic>(),
                cancellationToken,
                includeAutoGenerated,
        ...
        OutputKind outputKind = OutputKind.DynamicallyLinkedLibrary)
    {
        ...
        var loadDiagnostics = new List<Diagnostic>();
        IReadOnlyList<LoadedDocument> documents = BuildDocuments(
            compilation, projectDirectory: null, loadDiagnostics, includeAutoGenerated: false);
    ...
    private static async Task<LoadedCSharpProject> BuildLoadedProjectAsync(
        ...
        bool includeAutoGenerated,
        IReadOnlyCollection<string> includedAutoGeneratedPaths)
    {
        ...
        bool isTestProject = ReadIsTestProject(project);
    ...
    private static bool ReadIsTestProject(Project project)
    {
        return project.AnalyzerOptions.AnalyzerConfigOptionsProvider.GlobalOptions.TryGetValue(
            "build_property.IsTestProject",
            out string isTestProject) &&
            bool.TryParse(isTestProject, out bool value) &&
            value;
    }
        ...
        IReadOnlyCollection<string> includedAutoGeneratedPaths = null)
    {
        ...
        foreach (SyntaxTree tree in compilation.SyntaxTrees)
        {
            ...
            if (IsGeneratedSource(
                tree, projectDirectory, includeAutoGenerated, includedAutoGeneratedPaths))
            {
        ...
        IReadOnlyCollection<string> includedAutoGeneratedPaths)
    {
        ...
        // The rule itself lives in GeneratedSourceDetection, shared with the
        // translator; only the includeAutoGenerated opt-in is loader-specific.
        if (GeneratedSourceDetection.IsUnderBuildOutputDirectory(filePath, projectDirectory))
        {
            return true;
        }

        return GeneratedSourceDetection.HasAutoGeneratedHeader(tree) &&
            (!includeAutoGenerated ||
                (includedAutoGeneratedPaths is not null &&
                    !includedAutoGeneratedPaths.Contains(
