    ...
    public void EndToEnd_PrimaryCtorClass_ForwardsBaseArgsToClrBase_Runs()
    {
        const string source = """
            ...
            func Main() {
                ...
                // ADR-0186 step 4: `Exception.InnerException` is declared
                // `Exception?` by the annotated BCL, so reading through it is a
                // dereference of a member its author said may be nil. Member
                // lookup used to wave that through — the carve-out step 4
                // deletes could not tell an annotated-nullable member from an
