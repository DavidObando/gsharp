...
public sealed class GoldenFileTests : IDisposable
{
    ...
    [Fact]
    public void UpdateModeAcceptsGeneratedOutput()
    {
        string goldenPath = Path.Combine(this.directory, "sample.golden");
        string previous = Environment.GetEnvironmentVariable("GSHARP_UPDATE_GOLDENS");
        Environment.SetEnvironmentVariable("GSHARP_UPDATE_GOLDENS", "1");
        try
        {
        ...
        {
            Environment.SetEnvironmentVariable("GSHARP_UPDATE_GOLDENS", previous);
        }
