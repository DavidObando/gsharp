// <copyright file="AddressBookSampleTests.cs" company="GSharp">
...
/// </summary>
public class AddressBookSampleTests
{
    [Fact]
    public void AddressBookSample_RunsOnInterpreter_MatchesGolden()
    {
        var samplesDir = LocateSamplesDirectory();
        Assert.NotNull(samplesDir);

        var samplePath = Path.Combine(samplesDir!, "AddressBook.gs");
        var goldenPath = Path.Combine(samplesDir!, "AddressBook.golden");
    ...
    {
        var dir = new DirectoryInfo(Path.GetDirectoryName(typeof(AddressBookSampleTests).Assembly.Location) !);
        while (dir != null)
        {
